...
class Plotter():
    ...
    def twoDplot(self,classname,*args):
        ...
        if classname == "SolvencyRatio":
            ...
            axs[0].legend()
            axs[1].bar(self.xaxis, np.flip(self.int_cov_ratio[:-1]), label="Interest coverage ratio", color='purple')
            #axs[1].grid(True)
            ...
            axs[1,0].plot(self.xaxis, np.flip(self.net_prft_margin[:-1]), label="Net Profit Margin", color='green', marker='o')
            axs[1,0].grid(True)
            ...
            axs[1,1].plot(self.xaxis, np.flip(self.opr_income_margin[:-1]), label="Operating Income Margin", color='purple', marker='o')
            axs[1,1].grid(True)
